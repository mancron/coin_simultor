    ...
    public void executeMarketOrder(String userId, int sideIdx, BigDecimal amountOrQty) throws Exception {
        ...
        try (Connection conn = dataSource.getConnection()) {
            try {
                ...
                // 1. 매수 프로세스
                if (sideIdx == 0) { 
                    // [수정] 매수 시 입력값(amountOrQty)은 KRW 총액이므로 여기서 5000원 검증
                    if (amountOrQty.compareTo(new BigDecimal("5000")) < 0) {
                        throw new RuntimeException("최소 매수 금액은 5,000원 이상이어야 합니다.");
                    }
                    ...
                    // [수정] 원화(KRW)는 소수점이 없어야 하므로 버림 처리
                    BigDecimal actualInvestment = amountOrQty.subtract(fee).setScale(0, RoundingMode.DOWN); 
                    // 매수 수량 = 실제 투입 금액 / 현재가
                    BigDecimal buyQty = actualInvestment.divide(currentMarketPrice, 8, RoundingMode.DOWN);

                    // 자산 업데이트 (Atomic Update)
                    repo.updateBalance(conn, userId, "KRW", amountOrQty.negate()); // 총 주문금액(KRW) 차감
                    repo.updateBalance(conn, userId, "BTC", buyQty);               // BTC 증가

                    // 기록 저장 (상태: DONE, 타입: MARKET)
                ...
                // 2. 매도 프로세스
                    ...
                    // [수정] 총 매도 금액 = 주문 수량 * 현재가
                    BigDecimal totalSaleAmount = amountOrQty.multiply(currentMarketPrice);
                    
                    // [수정] 매도 시 환산된 원화 금액(totalSaleAmount)으로 5000원 검증
                    if (totalSaleAmount.compareTo(new BigDecimal("5000")) < 0) {
                        throw new RuntimeException("최소 매도 금액은 5,000원 이상이어야 합니다.");
                    }
                    ...
                    // [수정] 원화(KRW) 정산이므로 소수점 버림 처리
                    BigDecimal settleAmount = totalSaleAmount.subtract(fee).setScale(0, RoundingMode.DOWN);
                    ...
                    // 기록 저장 (상태: DONE)
                    String orderId = "ORD-" + System.currentTimeMillis();
                    repo.saveOrder(conn, orderId, userId, "MARKET", "ASK", "DONE", currentMarketPrice, amountOrQty);
                }
